...
function Job (jobData) {
  ...
  // If job type is not defined, assume it's a Basic job
  self.jobType = self.jobType || 'Basic';

  // Populate the job's build sequence
  app.getAllJobTypes()[self.jobType](self);
}
 ...
 * @param {Function} callback Signature: err, job
 */
Job.createJob = function (jobData, callback) {
  var validationErrors = Job.validate(jobData)
    , j;
 ...
 */
Job.removeJob = function (name, cb) {
  var callback = cb || function () {};

  db.jobs.findOne({ name: name }, function (err, job) {
    if (err) { return callback(err); }
    if (!job) { return callback("Job doesn't exist"); }

    db.jobs.remove({ name: name }, { multi: false }, function (err) {
      if (err) { return callback(err); }

      rimraf(Job.getRootDir(name), function (err) { return callback(err); });
    });
  });
};
...
Job.getJob = function (name, callback) {
  db.jobs.findOne({ name: name }, function (err, jobData) {
    if (err) { return callback (err); }
    if (!jobData) { return callback(null, null); }
 ...
 * Side effects are the stuff Cranium needs to do on top of just changing the Job object's properties
...
Job.getEditableProperties = function () {
  ...
  return ['name', 'branch', 'githubRepoUrl', 'repoSSHUrl', 'testScript', 'deployScript', 'jobType'];
};
...
Job._edit.repoSSHUrl = function (newValue, callback) {
  ...
  rimraf(Job.getRepoPath(self.name), function (err) {
    if (err) { return callback(err); }
    rimraf(Job.getDependenciesInfoDir(self.name), function (err) {
      if (err) { return callback(err); }
...
Job.prototype.edit = function (newOptions, callback) {
  var keys = Object.keys(newOptions)
    ...
    , validationErrors = Job.validate(newOptions)
    , i = 0
    ;
...
Job.prototype.advertiseOnHipchat = function (buildSuccessful) {
  var messageToSend = { from: 'Cranium CI'
    ...
    , self = this
    , buildUrl = '/jobs/' + self.name + '/builds/' + self.nextBuildNumber
