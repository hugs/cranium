...
module.exports.serveFavicon = function (req, res, next) {
  if (req.url === '/favicon.ico') {
    return res.send(404);   // I will change this once I have a logo for Cranium
  } else {
    return next();
  }
};
...
module.exports.commonRenderValues = function (req, res, next) {
  ...
  req.renderValues.currentProjectName = executor.getCurrentProject() && executor.getCurrentProject().name;
  req.renderValues.idle = req.renderValues.currentProjectName === undefined || req.renderValues.currentProjectName === null;
  req.renderValues.queuedProjects = executor.getQueueState();
  req.renderValues.someProjectsQueued = req.renderValues.queuedProjects.length > 0;

  db.projects.find({}, function (err, projects) {
    req.renderValues.projectNames = _.pluck(projects, 'name');
    return next();
  });
};
